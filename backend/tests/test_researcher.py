...
def test_run_researcher_returns_evidence(
    mock_retrieve, mock_web_search, mock_image_search, mock_video_search, mock_llm_client
):
...
def test_run_researcher_returns_empty_when_no_material_found(
    mock_retrieve, mock_web_search, mock_image_search, mock_video_search
):
    mock_retrieve.return_value = []
    mock_web_search.return_value = []
    mock_image_search.return_value = []
...
def test_run_researcher_returns_videos(
    ...
    mock_image_search.return_value = []
    mock_video_search.return_value = [
        {"title": "Video by Jane", "url": "https://pexels.com/video/1", "preview_video_url": "https://videos.pexels.com/1.mp4"}
    ]
    mock_llm_client.generate_json.return_value = {
        "evidence": [
            {"claim": "A claim", "source_title": "Source", "type": "evidence", "confidence": "high"}
